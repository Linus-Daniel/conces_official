...
export async function GET(request: NextRequest) {
  ...
  try {
    const profile = await AlumniProfile.findOne({ userId: session.user.id }).populate(
      'userId',
      'fullName email avatar role branch'
    );
...
export async function POST(req: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to save alumni profile' }, { status: 500 });
  }
}
