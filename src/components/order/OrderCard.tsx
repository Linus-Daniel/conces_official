      ...
      <div className="p-6">
        <div className="flex flex-col sm:flex-row justify-between gap-4">
          <div>
            <div className="flex items-center">
              <h3 className="text-lg font-medium text-royal-900">Order #{order.number}</h3>
        ...
        <div className="mt-6 flex flex-col sm:flex-row justify-between gap-3">
          <div>
            <p className="text-sm text-gray-500">
              {order.status === 'Delivered' 
                ...
                ? `Estimated delivery: `
                : `Estimated ship date: `}
              <span className="font-medium text-gray-700">{order.deliveryDate}</span>
            </p>
          </div>
          <div className="flex space-x-3">
            ...
            {order.status === 'Shipped' && (
              <a href={`/track-order/${order.id}`} className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-royal-DEFAULT hover:bg-royal-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-royal-500">
